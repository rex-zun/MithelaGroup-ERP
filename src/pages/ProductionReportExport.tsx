...
import { fetchMachines, fetchDepartments, fetchMasterDetails } from '@/lib/supabaseData';
import type { Machine, Department, MasterDetail } from '@/types';
...
export default function ProductionReportExport() {
  ...
  useEffect(() => {
    Promise.all([fetchMachines(), fetchDepartments(), fetchMasterDetails()]).then(([m, d, md]) => {
      setMachines(m);
      setDepartments(d);
      setMasterDetails(md);
    }).catch(console.error);
  ...
  const handleSearch = () => {
    toast.success('Search Applied', { description: 'Filters have been applied to the report' });
  };

  const handleExport = () => {
    toast.success(`${exportType === 'excel' ? 'Excel' : 'PDF'} Export`, {
      description: `Production report is being exported as ${exportType.toUpperCase()}`,
    });
  };
              ...
              <div className="text-center space-y-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    {Object.values(advancedSearch).some((v) => v) || dateFrom || selectedMachine
